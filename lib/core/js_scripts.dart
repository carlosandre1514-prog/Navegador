...
class JsScripts {
  ...
  // ---------------------------------------------------------------------------
  // Modo TV: impede que o teclado virtual do sistema apareça ao tocar em
  // campos da página. Isso não afeta um teclado físico USB — ele funciona
  // por eventos de tecla, que não dependem do teclado na tela.
  // ---------------------------------------------------------------------------

  static const String semTecladoNaTela = r'''
    (function () {
      if (window.__semTecladoNaTela) return;
      window.__semTecladoNaTela = true;
      function aplicar(el) {
        if (!el || el.__semTeclado) return;
        var tag = (el.tagName || '').toUpperCase();
        if (tag === 'INPUT' || tag === 'TEXTAREA' || el.isContentEditable) {
          try { el.setAttribute('inputmode', 'none'); } catch (e) {}
          el.__semTeclado = true;
        }
      }
      function varrer(raiz) {
        if (!raiz || !raiz.querySelectorAll) return;
        aplicar(raiz);
        raiz.querySelectorAll('input, textarea, [contenteditable]').forEach(aplicar);
      }
      varrer(document);
      try {
